...
const StampSeal = () => {
    ...
    // Fast-path: prefer restoring UI state from location.state.cartRow when variantGroups are available.
    useEffect(() => {
        const tryHydrateFromNav = () => {
            try {
                ...
                console.debug('[Stamp-Seal] tryHydrateFromNav failed', e);
            }
            return false;
        };
    ...
    useEffect(() => {
        ...
        const buildThumbnails = async () => {
            const thumbnails = [undefined, undefined, undefined, undefined];

            // Get all thumbnail URLs in parallel for better performance
            const [
                mainImageUrl,
                waxStampUrl,
                stamp1Url,
                ...
                imageKey ? tryGetPublic('3d-prints-images', imageKey) : Promise.resolve(null),
                tryGetPublic('3d-prints-images', 'wax-stamp'),
                tryGetPublic('3d-prints-images', 'wax-stamp-1'),
                tryGetPublic('3d-prints-images', 'wax-stamp-2'),
                tryGetPublic('3d-prints-images', 'wax-stamp-3')
            ]);

            // 1. First thumbnail: wax-stamp.png
            if (waxStampUrl) {
                thumbnails[0] = waxStampUrl;
            } else if (imageSrc && imageSrc.startsWith('http')) {
                thumbnails[0] = imageSrc;
            } else if (mainImageUrl) {
                thumbnails[0] = mainImageUrl;
            }

            // 2. Second thumbnail: wax-stamp-1
            if (stamp1Url) thumbnails[1] = stamp1Url;

            // 3. Third thumbnail: wax-stamp-2
            if (stamp2Url) thumbnails[2] = stamp2Url;

            // 4. Fourth thumbnail: wax-stamp-3
            if (stamp3Url) thumbnails[3] = stamp3Url;

            // If main image is still not set, try to get it from product data
            if (!thumbnails[0] && productId) {
                try {
                    ...
                    if (!error && productData?.image_url) {
                        const mainImageUrl = tryGetPublic('3d-prints-images', productData.image_url);
                        if (mainImageUrl) thumbnails[0] = mainImageUrl;
                    }
                } catch (err) {
                    console.warn('[Stamp-Seal-Info] Error fetching main product image:', err);
                }
            }

            // Fill any remaining undefined slots with fallbacks
            const fallbacks = ['/3d-prints/stamp-seal.png', '/3d-prints/stamp-seal-1.png', '/3d-prints/stamp-seal-2.png', '/logo-icon/logo.png'];
            for (let i = 0; i < thumbnails.length; i++) {
                if (thumbnails[i]) continue;

                // Try the corresponding fallback
                if (i < fallbacks.length) {
                    try {
                        const exists = await headExists(fallbacks[i]);
                        if (exists) {
                            thumbnails[i] = fallbacks[i];
                        }
                    } catch (e) {
                        // ignore
                    }
            ...
            // Set default to first thumbnail only if not already set, preserving user selections
            setActiveThumb(prev => prev === undefined || prev === null ? 0 : prev);
            // Also set the main image to the first thumbnail only if not already set
            if (thumbnails[0] && (!imageSrc || imageSrc === '/3d-prints/stamp-seal.png')) {
                setImageSrc(thumbnails[0]);
            }
        };
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Stamp-Seal-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Stamp-Seal-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Clothing-Banner-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    // If the product has no visible variant groups, attempt to derive stock by
    // looking up product_variant_combinations for the product and summing
    // inventory rows linked to those combination_ids.
    useEffect(() => {
        const fetchStockInfo = async () => {
            ...
            // If there are no variant groups presented in the UI, aggregate inventory
            // across all combinations for this product so stock can be shown.
            if (!variantGroups || variantGroups.length === 0) {
                try {
                    ...
                    if (combError) {
                        console.warn('[Stamp-Seal] product_variant_combinations query failed', combError);
                        setStockInfo(null);
                        return;
                    }

                    const combinationIds = (combinations || []).map(c => c.combination_id).filter(Boolean);
                    ...
                    if (invErr) {
                        console.warn('[Stamp-Seal] inventory lookup failed', invErr);
                        setStockInfo(null);
                        return;
                    }
                    ...
                    console.error('[Stamp-Seal] unexpected error aggregating inventory for product:', err);
                    setStockInfo(null);
                    return;
                }
            }

            // Otherwise, match selectedVariants to product_variant_combinations as before
    ...
    useEffect(() => {
        ...
        const resolveImage = async () => {
            ...
            // If imageKey is not provided, use the default stamp seal image
            if (!imageKey) {
                if (isMounted) setImageSrc('/3d-prints/stamp-seal.png');
                return;
            }
            ...
            try {
                if (/^https?:\/\//i.test(imageKey) || imageKey.startsWith('/')) {
                    ...
                    console.debug('[Stamp-Seal-Info] using provided imageKey as src', { imageKey });
                    return;
                }

                const cleanKey = String(imageKey).replace(/^\/+/, ''); // remove leading slash(es)

                // Try buckets with 3d-prints-images prioritized for 3D print products
                const bucketsToTry = ['3d-prints-images', 'product-images', 'images', 'signage-posters-images', 'accessoriesdecorations-images', 'apparel-images', 'public'];
                for (const bucket of bucketsToTry) {
                    try {
                        const { data, error } = supabase.storage.from(bucket).getPublicUrl(cleanKey);
                        console.debug('[Stamp-Seal-Info] getPublicUrl attempt', { bucket, cleanKey, data, error });
                        ...
                        console.warn('[Stamp-Seal-Info] bucket attempt failed', { bucket, err });
                        // continue trying other buckets
                    }
                }

                // Last-resort fallback to local public asset for 3D prints
                if (isMounted) setImageSrc('/3d-prints/stamp-seal.png');
                console.warn('[Stamp-Seal-Info] could not resolve imageKey to a public URL, using fallback', { imageKey });
            } catch (err) {
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Clothing-Banner-Info] reviews query error:', error.message || error);
                ...
                console.error('[Clothing-Banner-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
    ...
    const incrementQuantity = () => setQuantity((q) => {
        const stock = typeof stockInfo?.quantity === 'number' ? Number(stockInfo.quantity) : null;
        if (stock != null) return Math.min(q + 1, stock);
        return q + 1;
    });
    const decrementQuantity = () => setQuantity((q) => Math.max(1, q - 1));

    // Ensure quantity never exceeds fetched stock when stockInfo updates
    useEffect(() => {
        const stock = typeof stockInfo?.quantity === 'number' ? Number(stockInfo.quantity) : null;
        if (stock != null) {
            setQuantity(prev => Math.min(Math.max(1, prev || 1), stock));
        }
    }, [stockInfo]);

    // Clear the transient "*Required" hint as soon as an uploaded file is present
    useEffect(() => {
        if (uploadedFileMetas && uploadedFileMetas.length > 0 && uploadRequired) {
            setUploadRequired(false);
        }
    }, [uploadedFileMetas, uploadRequired]);
    ...
    const handleAddToCart = async () => {
        ...
        // Require an uploaded design file for this product (UploadDesign populates uploadedFileMetas)
        if (showUploadUI && (!uploadedFileMetas || uploadedFileMetas.length === 0)) {
            // Show a short-lived hint next to the upload label instead of an error by the Add to Cart button
            setUploadRequired(true);
            setTimeout(() => setUploadRequired(false), 2000);
            return;
        }
    ...
    console.log('StampSeal component rendering, imageSrc:', imageSrc);
    console.log('StampSeal component rendering, imageKey:', imageKey);
            ...
            <div className="max-w-[1201px] mx-auto mt-8 flex flex-col">
                ...
                <div className="flex flex-col tablet:flex-row laptop:gap-2 tablet:gap-[50px] phone:p-2 tablet:p-2 justify-center w-full items-stretch">
                                        ...
                                        if (src) {
                                            ...
                                            cells.push(
                                                <button
                                                    ...
                                                    className={`border rounded p-1 overflow-hidden flex items-center justify-center outline-none ${isActive ? 'ring-2 ring-offset-1 ring-black focus:outline-none' : ''}`}
                                                    style={{ width: 120, height: 135 }}
                                                >
                                                    <img
                                                        src={src}
                                                        alt={`Thumbnail ${i + 1}`}
                                                        className={`h-full w-full object-cover transition-transform duration-200 focus:outline-none ease-in-out transform ${isActive ? 'scale-110' : 'hover:scale-110'}`}
                                                    />
                                                </button>
                                            );
                                        } else {
                    ...
                    <div className="border border-black rounded-md p-6 w-full tablet:w-[601px] h-[732px] flex flex-col overflow-y-auto pr-2">
                        ...
                        <div className="mb-2">
                            {( (!variantGroups || variantGroups.length === 0) || Object.values(selectedVariants).filter(v => v?.id).length > 0 ) ? (
                                stockInfo ? (
                                    stockInfo.quantity === 0 ? (
                                        <span className="text-red-600 font-semibold">Out of Stocks</span>
                                    ) : (
                                        <span className="text-black font-semibold">{(Number(stockInfo.quantity) === 1) ? 'Stock:' : 'Stocks:'} {stockInfo.quantity}</span>
                                    )
                                ) : (
                        ...
                        <div className="mb-6">
                            <div className="text-[16px] font-semibold text-gray-700 mb-2">
                                UPLOAD DESIGN {uploadRequired && <span className="text-red-600 ml-2 text-sm">*Required</span>}
                            </div>
                        ...
                        <div className="mb-6">
                            ...
                            <div className="inline-flex items-center border border-blaack rounded">
                                ...
                                <button type="button" className="px-3 bg-white text-black focus:outline-none focus:ring-0" onClick={incrementQuantity} aria-label="Increase quantity" disabled={typeof stockInfo?.quantity === 'number' ? quantity >= Number(stockInfo.quantity) || Number(stockInfo.quantity) === 0 : false}>+</button>
                            </div>
                        </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-right text-sm text-gray-500 mt-1">
                                        {String(reviewText?.length || 0)}/300
                                    </div>
                                </div>
