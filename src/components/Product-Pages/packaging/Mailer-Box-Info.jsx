...
const MailerBox = () => {
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Mailer-Box-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Mailer-Box-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Mailer-Box-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    // Build thumbnails: start with local mailer-box, then storage variants, derived, and fallbacks
    useEffect(() => {
        let isMounted = true;
        ...
        const buildThumbnails = async () => {
            const results = [];
            results.push('/packaging/mailer-box.png');
            const desired = ['mailer-box-1', 'mailer-box-2', 'mailer-box-3'];
            ...
            const fallbacks = ['/packaging/mailer-box.png', '/packaging/mailer-box-1.png', '/packaging/mailer-box-2.png', '/logo-icon/logo.png'];
            ...
            setActiveThumb(prev => { if (padded[prev]) return prev; const i = padded.findIndex(Boolean); return i === -1 ? 0 : i; });
            const i = padded.findIndex(Boolean); if (i >= 0 && !imageSrc) setImageSrc(padded[i]);
        };
        buildThumbnails();
        return () => { isMounted = false; };
    }, [imageKey, imageSrc]);
    ...
    // Prefer navigation-state hydration (location.state.cartRow.variants) when present to avoid extra DB roundtrips.
    useEffect(() => {
        ...
        const tryHydrateFromNav = () => {
            ...
            try {
                ...
                for (const nv of navVars) {
                    ...
                    // Try to match by product_variant_value_id, then by value/name
                    let match = null;
                    if (nv.product_variant_value_id) {
                        match = group.values.find(v => String(v.id) === String(nv.product_variant_value_id));
                    }
                    if (!match) {
                        const compare = String(nv.value || nv.name || nv.value_name || '').toLowerCase();
                ...
                console.debug('Nav hydration failed (MailerBox):', e);
            }
            return false;
        };

        if (tryHydrateFromNav()) return; // done
    ...
    const handleAddToCart = async () => {
        ...
        try {
            ...
            try {
                ...
                console.warn('Failed to attach uploaded_files to cart row (Mailer-Box):', e);
            }
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Mailer-Box-Info] reviews query error:', error.message || error);
                ...
                console.error('[Mailer-Box-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
                ...
                <div className="border border-black rounded-md overflow-hidden">
                    ...
                    >
                        <ul className="text-sm text-gray-700 space-y-2 text-black text-[16px] font-dm-sans">
                            ...
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Size:  4x4x2 inches, 5.5x5.5x2.5 inches, 6.5x6.5x3 inches</p></li>
