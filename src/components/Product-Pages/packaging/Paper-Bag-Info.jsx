...
const PaperBag = () => {
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Paper-Bag-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Paper-Bag-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Paper-Bag-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    useEffect(() => {
        const fetchStockInfo = async () => {
            ...
            // Require a selection for every group
            ...
            // 1) Find matching combination for this product
            ...
            // 2) Query inventory for that combination
    ...
    // Build thumbnails for Paper-Bag
    useEffect(() => {
        let isMounted = true;
        ...
        const buildThumbnails = async () => {
            const results = [];
            results.push('/packaging/paper-bag.png');
            for (const base of ['paper-bag-1','paper-bag-2','paper-bag-3']) {
            ...
            for (const f of ['/packaging/paper-bag.png','/packaging/paper-bag-1.png','/packaging/paper-bag-2.png','/logo-icon/logo.png']) {
                if (results.length >= 4) break;
    ...
    const handleAddToCart = async () => {
        ...
        try {
            ...
            try {
                ...
                console.warn('Failed to attach uploaded_files to cart row (Paper-Bag):', e);
            }
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Paper-Bag-Info] reviews query error:', error.message || error);
                ...
                console.error('[Paper-Bag-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
                ...
                <div className="border border-black rounded-md overflow-hidden">
                    ...
                    >
                        <ul className="text-sm text-gray-700 space-y-2 text-black text-[16px] font-dm-sans">
                            ...
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Size: 15x8x21 cm, 21x11x27 cm</p></li>
