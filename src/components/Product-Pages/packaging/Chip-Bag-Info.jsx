...
const ChipBag = () => {
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Chip-Bag-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Chip-Bag-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Chip-Bag-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    useEffect(() => {
        ...
        const fetchStockInfo = async () => {
            ...
            // Require at least one variant selected; do not require all groups
    ...
    // Clamp quantity if it exceeds stock when stockInfo changes
    useEffect(() => {
        if (stockInfo && typeof stockInfo.quantity === 'number') {
            setQuantity(q => {
                if (stockInfo.quantity <= 0) return q; // keep at least 1; add-to-cart is disabled anyway
    ...
    // Prefer navigation-state hydration when available
    useEffect(() => {
        if (!fromCart || !editingCartId) return;

        const tryHydrateFromNav = () => {
            const navRow = location?.state?.cartRow;
            const navVars = navRow?.variants || navRow?.cart_variants;
            if (!navVars || !Array.isArray(navVars) || !variantGroups || variantGroups.length === 0) return false;

            try {
                const restored = {};
                for (const nv of navVars) {
                    const groupName = String(nv.group || nv.variant_groups?.name || '').trim().toLowerCase();
                    ...
                    let match = null;
                    if (nv.product_variant_value_id) {
                        match = group.values.find(v => String(v.id) === String(nv.product_variant_value_id));
                    }
                    if (!match) {
                        const compare = String(nv.value || nv.name || '').toLowerCase();
                        match = group.values.find(v => String(v.name || v.value || '').toLowerCase() === compare || String(v.name || '').toLowerCase().includes(compare));
                    }
                    if (match) restored[group.id] = match;
                }
                if (Object.keys(restored).length > 0) {
                    setSelectedVariants(restored);
                    if (navRow.quantity) setQuantity(navRow.quantity);
                    return true;
                }
            } catch (e) {
                console.debug('Nav hydration failed (ChipBag):', e);
            }
            return false;
        };
        ...
        restoreFromCart();
    }, [fromCart, editingCartId, variantGroups, location]);
    ...
    // Build thumbnails: 1) first thumb from local packaging asset, 2) try storage variants (-1,-2,-3), 3) derive from imageKey base, 4) local fallbacks
    useEffect(() => {
        let isMounted = true;
        ...
        const buildThumbnails = async () => {
            ...
            // 1) Always start with local main packaging asset
            results.push('/packaging/chip-bag.png');

            // 2) Desired storage variants from 'packaging-images'
            const desired = ['chip-bag-1', 'chip-bag-2', 'chip-bag-3'];
            ...
            // 3) Derive based on imageKey if provided
            if (results.length < 4 && imageKey) {
                ...
                for (const suffix of ['-1', '-2', '-3']) {
                    if (results.length >= 4) break;
                    const url = await tryGetPublic('packaging-images', base + suffix);
                    if (url) results.push(url);
                }
            }

            // 4) Local public fallbacks
            const fallbacks = ['/packaging/chip-bag.png', '/packaging/chip-bag-1.png', '/packaging/chip-bag-2.png', '/logo-icon/logo.png'];
            ...
            // Deduplicate, cap to 4, and pad with undefined
            const seen = new Set();
            const ordered = [];
            ...
            setActiveThumb(prev => {
                ...
                const idx = padded.findIndex(Boolean);
                return idx === -1 ? 0 : idx;
            });
            // Ensure main image reflects active thumbnail if none set
            const activeIdx = padded.findIndex(Boolean);
            if (activeIdx >= 0 && !imageSrc) setImageSrc(padded[activeIdx]);
        };
    ...
    const handleAddToCart = async () => {
        ...
        try {
            ...
            try {
                ...
                console.warn('Failed to attach uploaded_files to cart row (Chip-Bag):', e);
            }
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            if (!productId) return;
            console.log('[Chip-Bag-Info] fetching reviews for product:', productId);
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Chip-Bag-Info] reviews query error:', error.message || error);
                    ...
                    console.log('[Chip-Bag-Info] fetched reviews:', data.length);
                    setReviews(data);
                    ...
                    if (userIds.length > 0) {
                        ...
                        if (!ordersError && orders) {
                            const verified = {};
                            orders.forEach(o => {
                                verified[o.user_id] = true;
                            });
                            setVerifiedBuyerMap(verified);
                        }
                    }
                } else {
                    console.log('[Chip-Bag-Info] no reviews found');
                    setReviewsAvailable(true);
                    setReviews([]);
                    setReviewsCount(0);
                    setAverageRating(null);
                ...
                console.error('[Chip-Bag-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviews([]);
                setReviewsCount(0);
                setAverageRating(null);
    ...
    const onPickReviewFiles = (e) => {
        ...
        files.forEach(f => {
            ...
            } else if (!['image/png', 'image/jpeg', 'image/svg+xml'].includes(f.type) && !f.name.toLowerCase().endsWith('.svg')) {
    ...
    const submitReview = async () => {
        ...
        try {
            ...
            console.log('[Chip-Bag-Info] review submitted successfully');
            // Refresh reviews
            const { data: newReviews, error: refetchError } = await supabase
                .from('user_reviews')
                .select(`
                    id,
                    user_id,
            ...
            console.error('[Chip-Bag-Info] error submitting review:', err);
            ...
            <div className="max-w-[1201px] mx-auto mt-8 flex flex-col">
                ...
                <div className="flex flex-col tablet:flex-row laptop:gap-2 tablet:gap-[50px] phone:p-2 tablet:p-2 justify-center w-full items-stretch">
                            ...
                            <div className="mt-4 grid grid-cols-4 gap-3">
                                {thumbnails.map((src, i) => (
                                    <button
                                        key={i}
                                        type="button"
                                        className={`h-[135px] w-[120px] border rounded p-2 bg-[#f7f7f7] flex items-center justify-center focus:outline-none ${i === activeThumb ? 'ring-2 ring-black' : ''}`}
                                        onClick={() => { if (src) { setActiveThumb(i); setImageSrc(src); } }}
                                        disabled={!src}
                                        aria-label={`Thumbnail ${i + 1}`}
                                    >
                                        {src ? (
                                            <img src={src} alt="Thumbnail" className="object-contain h-full w-full" />
                                        ) : (
                    ...
                    <div className="border border-black rounded-md p-6 w-full tablet:w-[601px] h-[732px] flex flex-col overflow-y-auto pr-2">
                        ...
                        <div className="mb-6">
                            <div className="text-[16px] font-semibold text-gray-700 mb-2">QUANTITY (THE PURCHASE UNIT IS 5 PIECES)</div>
                            {quantityGroup && (
                                <div className="flex flex-wrap gap-3">
                ...
                <div className="border border-black rounded-md overflow-hidden">
                    ...
                    >
                        <ul className="text-sm text-gray-700 space-y-2 text-black text-[16px] font-dm-sans">
                            ...
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Material: Polypropylene</p></li>
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Size: 4x5 inches, 5x8 inches</p></li>
