    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Plastic-Bag-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Plastic-Bag-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Plastic-Bag-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    useEffect(() => {
        ...
        const tryHydrateFromNav = () => {
            ...
            try {
                ...
                console.debug('Nav hydration failed (PlasticBag):', e);
            }
            return false;
        };
        ...
        const restoreCartData = async () => {
            try {
                ...
                if (cartData?.quantity) setQuantity(cartData.quantity);

                // Fetch variants associated with cart
                const { data: cartVariants, error: varError } = await supabase
                    .from('cart_variants')
                    .select('cartvariant_id, variant_values (id, name, variant_groups (id, name), price)')
                    .eq('cart_id', editingCartId);
                ...
                (cartVariants || []).forEach(cv => {
                    if (cv.variant_values?.variant_groups?.id) {
                        restored[cv.variant_values.variant_groups.id] = cv.variant_values;
                    }
                });
                setSelectedVariants(restored);
            } catch (err) {
    ...
    const handleAddToCart = async () => {
        ...
        try {
            ...
            try {
                ...
                console.warn('Failed to attach uploaded_files to cart row (Plastic-Bag):', e);
            }
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Plastic-Bag-Info] reviews query error:', error.message || error);
                ...
                console.error('[Plastic-Bag-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
                ...
                <div className="border border-black rounded-md overflow-hidden">
                    ...
                    >
                        <ul className="text-sm text-gray-700 space-y-2 text-black text-[16px] font-dm-sans">
                            ...
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Material: Plastic</p></li>
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Size: 15x22 cm, 20x30 cm</p></li>
