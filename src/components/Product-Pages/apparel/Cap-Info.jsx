...
const Cap = () => {
            ...
            try {
                ...
                pvvData.forEach((pvv) => {
                    ...
                    if (vv && vv.variant_groups) {
                        ...
                        if (
                            !groupEntry.values.some(
                                (v) => v.id === pvv.product_variant_value_id
                            )
                        ) {
                            groupEntry.values.push({
                                ...
                                variant_value_id: pvv.variant_value_id,
                            });
                        }
                    }
                });
        ...
        try {
            if (!prev) {
                const { data, error } = await supabase
                    ...
                    .select();
                console.log("[Cap-Info] insert favorite response:", {
                    data,
                    error,
                });
                setLastFavResp({
                ...
                if (error) {
                    if (
                        ...
                        console.warn(
                            "[Cap-Info] insert conflict (already exists), treating as success",
                            error
                        );
                    } else {
                        throw error;
                ...
                const { data, error } = await supabase
                    ...
                    .eq("product_id", productId)
                    .select();
                console.log("[Cap-Info] delete favorite response:", {
                    data,
                    error,
                });
                setLastFavResp({
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                console.error(
                    "[Cap-Info] unexpected error fetching reviews:",
                    err
                );
                setReviewsAvailable(false);
                setReviewsCount(0);
    ...
    // Upload handler for design files
            ...
            if (meta) {
                if (meta.id)
                    ...
                    await supabase
                        .from("uploaded_files")
                        .delete()
                        .match({ user_id: userId, file_name: meta.file_name });
            }
        } catch (err) {
    ...
    useEffect(() => {
        ...
        uploadedFileMetas.forEach((m, i) => {
            if (
                ...
                uploadedFiles[i].type.startsWith("image/")
            ) {
                const o = URL.createObjectURL(uploadedFiles[i]);
                objectUrls.push(o);
                urls[i] = o;
            } else urls[i] = null;
        });
        for (
            let i = uploadedFileMetas.length;
    ...
    // Fetch latest uploaded file for the current user
    useEffect(() => {
        let isMounted = true;
        const fetchUploadedFiles = async () => {
            try {
                ...
                if (error) {
                    console.warn(
                        "[Cap-Info] could not order uploaded_files by uploaded_at:",
                        error.message || error
                    );
                    const fallback = await supabase
                        .from("uploaded_files")
                    ...
                    if (fallback.error) {
                        console.warn(
                            "[Cap-Info] fallback uploaded_files query failed:",
                            fallback.error
                        );
                        return;
                    }
    ...
    const handleAddToCart = async () => {
        ...
        try {
            ...
            for (const cart of existingCarts || []) {
                ...
                const existingVarSet = new Set(
                    (cartVariants || []).map((v) => `${v.cartvariant_id}`)
                );

                // Build a set of selected variant_value_ids from the UI
                const selectedVarSet = new Set(
                    Object.values(selectedVariants || {}).map((val) =>
                        `${val?.variant_value_id ?? val?.id ?? val}`
                    )
                );

                // Compare sets (treat both empty as equal)
                if (
                    existingVarSet.size === selectedVarSet.size &&
                    [...existingVarSet].every((v) => selectedVarSet.has(v))
                ) {
            ...
            if (!cartMatched) {
                ...
                console.log("Inserted cart_id:", cartId);

                // Insert into cart_variants for each selected variant
                const variantInserts = Object.entries(selectedVariants).map(
                    ([groupId, value]) => ({
                        cart_id: cartId,
                        user_id: userId,
                        cartvariant_id: value.variant_value_id, // Use variant_value_id to link variant
            ...
            // Skip updating uploaded_files with cart_id unless explicitly needed
            // Show success message and reset quantity
            setCartSuccess("Item added to cart!");
            setQuantity(1);
            setTimeout(() => setCartSuccess(null), 3000);
        } catch (err) {
            console.error("Error adding to cart - Details:", {
                message: err.message,
                code: err.code,
                details: err.details,
                stack: err.stack,
            ...
            if (err.code === "23505") {
                setCartError(
                    "This item with the same variants is already in your cart"
                );
            } else {
                setCartError(
    ...
    const techniqueGroup = variantGroups.find(
        (g) => g.name.toUpperCase() === "TECHNIQUE"
    );
    const colorGroup = variantGroups.find(
        (g) => g.name.toUpperCase() === "COLOR"
    );
            ...
            <div className="max-w-[1201px] mx-auto mt-8 flex flex-col">
                <div className="phone:p-2 tablet:p-2">
                    <p className="pt-5 font-dm-sans">
                        <Link to="/Homepage" className="text-gray-600">
                            Home{" "}
                        </Link>
                        /{" "}
                        <Link to="/apparel" className="text-gray-600">
                            Apparel{" "}
                        </Link>
                    </p>
                </div>
                ...
                <div className="flex flex-col tablet:flex-row laptop:gap-2 tablet:gap-[50px] phone:p-2 tablet:p-2 justify-center w-full items-stretch">
                    ...
                    <div className="bg-white w-full tablet:w-[573px] h-auto">
                        <div className="rounded-md p-6 h-full flex flex-col">
                            ...
                            <div className="mt-4 grid grid-cols-4 gap-3">
                                <div
                                    className="h-20 w-full border rounded p-2 bg-[#f7f7f7]"
                                    aria-hidden
                                />
                                <div
                                    className="h-20 w-full border rounded p-2 bg-[#f7f7f7]"
                    ...
                    <div className="border border-black rounded-md p-6 w-full tablet:w-[601px] h-full flex flex-col">
                        <h1 className="text-[36px] font-bold text-[#111233] mt-4 mb-2">
                            {loading ? "" : productName}
                        </h1>
                                ...
                                {reviewsAvailable
                                    ? reviewsCount > 0
                                        ? `${averageRating
                                              ...
                                              : "—"}
/5 ${reviewsCount} review${reviewsCount !== 1 ? "s" : ""}`
                                        : "(—/5) 0 reviews"
                                    : "(—/5) 0 reviews"}
                            </div>
                        </div>
                            ...
                            {techniqueGroup && (
                                <div className="flex gap-3">
                                    {techniqueGroup.values.map((val) => {
                                        const isSelected =
                                            selectedVariants[techniqueGroup.id]
                                                ?.id === val.id;
                                        if (techniqueGroup.input_type === "color") {
                                            return (
                                                <div
                                                    key={val.id}
                                                    className={`w-8 h-8 rounded-full cursor-pointer ${isSelected
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`px-4 py-2 rounded ${isSelected
                                                        ? "bg-gray-200 text-gray-500 font-bold border border-gray-500"
                                                        : "bg-white text-[#111233] border border-[#111233]"} focus:outline-none focus:ring-0`}
                                                    onClick={() =>
                                                        selectVariant(
                                                            techniqueGroup.id,
                                                            val
                            ...
                            {colorGroup && (
                                <div className="flex items-center gap-3">
                                    {colorGroup.values.map((val) => {
                                        const isSelected =
                                            selectedVariants[colorGroup.id]
                                                ?.id === val.id;
                                        const isHexColor =
                                        ...
                                        return (
                                            <div
                                                key={val.id}
                                                className={`w-8 h-8 rounded cursor-pointer ${isSelected
                                                    ? "ring-2 ring-blue-500"
                                                    : "ring-1 ring-gray-300"}`}
                                                style={{
                        ...
                        <div className="flex items-center gap-4">
                            <button
                                type="button"
                                className="bg-[#ef7d66] text-black py-3 rounded w-full tablet:w-[314px] font-semibold focus:outline-none focus:ring-0 hover:bg-orange-600"
            ...
            <div className="max-w-[1200px] mx-auto mt-8 w-full laptop:px-2 phone:p-2 tablet:p-2">
                <div className="border border-black rounded-md overflow-hidden">
                    <div className="w-full flex items-center justify-between bg-white p-3 tablet:p-4 border-b border-b-black">
                        <h2 className="text-[32px] font-bold text-[#111233]">
                            Product Details
                        </h2>
                        <button
                            type="button"
                        ...
                        >
                            <img
                                ...
                                alt=""
                                aria-hidden
                                className={`h-6 w-6 rounded object-cover transform transition-transform duration-200 ${detailsOpen
                                    ? ""
                                    : "rotate-180"}`}
                            />
                        </button>
                    ...
                    <div
                        ...
                        className={`bg-gray-50 overflow-hidden transition-all duration-300 ease-in-out ${detailsOpen
                            ? "p-4 tablet:p-6 max-h-[500px] opacity-100"
                    ...
                    >
                        <ul className="text-sm text-gray-700 space-y-2 text-black text-[16px] font-dm-sans">
                            <li>
                                <p className="m-0 font-normal text-black text-[16px] font-dm-sans">
            ...
            <div className="max-w-[1200px] mx-auto mt-8 w-full laptop:px-2 phone:p-2 tablet:p-2">
                <div className="border border-black rounded-md overflow-hidden p-6">
                    <h2 className="text-[32px] font-bold text-[#111233] inline-block pb-2">
                        Customer Reviews
                    </h2>
