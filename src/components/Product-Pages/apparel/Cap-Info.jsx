...
const Cap= () => {
    ...
    useEffect(() => {
        const tryHydrateFromNav = () => {
            try {
                ...
                setEditingCartId(cartRow.cart_id);
                return true;
            } catch (e) { console.debug('[tryHydrateFromNav][Cap] failed', e); return false; }
        };
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Cap-Info] recently_viewed update error:', updError);
                }

                if (Array.isArray(updData) && updData.length > 0) {
                    if (process.env.NODE_ENV !== 'production') {
                        console.debug('[Cap-Info] recently_viewed updated', { productId, userId, row: updData?.[0]?.id });
                    }
                ...
                if (insError) {
                    console.warn('[Cap-Info] recently_viewed insert error:', insError);
                } else {
                    if (process.env.NODE_ENV !== 'production') {
                        console.debug('[Cap-Info] recently_viewed inserted', { productId, userId, row: insData?.[0]?.id });
                    }
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Cap-Info] recently_viewed log error:', err);
            }
        };
    ...
    const [reviewAuthors, setReviewAuthors] = useState({}); // userId -> name
    const [verifiedBuyerMap, setVerifiedBuyerMap] = useState({}); // userId -> true
    ...
    const maskName = (input) => {
        try {
            ...
            if (s.length <= 2) return s; // nothing or too short to mask
            const first = s[0];
    ...
    // Parse Supabase timestamp robustly:
    // - If the string has no timezone info, interpret as UTC (append 'Z').
    //   This matches how Supabase typically stores server-side times and
    //   avoids local offset inflation for users ahead of UTC (e.g., +8).
    // - If the string has 'Z' or an offset, rely on native Date parsing.
    const parseReviewDate = (val) => {
        if (!val) return null;
        try {
            if (typeof val === 'string') {
                const s = val.trim();
                // If string already includes timezone info (Z or +/-HH:MM), parse directly
                if (/Z|[+\-]\d{2}:?\d{2}$/.test(s)) {
                    const d = new Date(s);
                    return Number.isNaN(d.getTime()) ? null : d;
                }
                // Match 'YYYY-MM-DD[ T]HH:MM:SS(.sss)?' without timezone -> treat as UTC by appending 'Z'
    ...
    const [lightboxItems, setLightboxItems] = useState([]); // array of image URLs
    ...
    // Format a human-friendly time-ago label with minutes/hours/days
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (error) {
                    ...
                    // Resolve author names using auth (RPC) with profiles fallback
                    const userIds = Array.from(new Set(data.map(r => r.user_id).filter(Boolean)));
                    if (userIds.length) {
                        const nameMap = {};
                        // 0) Primary: auth.users via secure RPC (works for anon if granted)
                        try {
                            ...
                            if (!authErr && Array.isArray(authNames)) {
                                ...
                                console.debug('[Cap-Info] auth names RPC error:', authErr.message || authErr);
                            }
                        } catch (e) { /* ignore rpc errors */ }

                        // 1) (Removed public view fallback to avoid 400s when view isn't deployed)

                        // 2) Fallback: profiles table (public)
                        try {
                        ...
                        } catch (e) { /* ignore profiles errors */ }

                        setReviewAuthors(nameMap);

                        // Resolve verified buyers (best-effort)
                        try {
    ...
    const openReviewForm = async () => {
        // Require login before writing a review
        const userId = session?.user?.id ?? await getCurrentUserId();
        if (!userId) { navigate('/signin'); return; }
    ...
    const onPickReviewFiles = (e) => {
        ...
        for (const file of files) {
            if (file.size > MAX_BYTES) {
                setReviewUploadError('File is too large');
            ...
            if (!isAllowed) {
                setReviewUploadError('File format not supported.');
                try { if (reviewFileInputRef.current) reviewFileInputRef.current.value = null; } catch {}
                return;
            }
            toAdd.push(file);
        ...
        setReviewFiles(prev => {
            ...
            if (combined.length > 3) {
                setReviewUploadError('You can upload up to 3 images.');
            }
    ...
    const removeReviewFileAt = (index) => {
    ...
    const submitReview = async () => {
        ...
        const userId = session?.user?.id ?? await getCurrentUserId();
        if (!userId) { navigate('/signin'); return; }
        if (!reviewRating || reviewRating < 1) return; // rating required
        setIsSubmittingReview(true);
        try {
            // Upload up to 3 images to the 'reviews' storage bucket and collect URLs
            ...
            for (let i = 0; i < Math.min(reviewFiles.length, 3); i++) {
                ...
                try {
                    ...
                    if (upErr) {
                        uploadFailures++;
                        console.warn('Review image upload error:', upErr?.message || upErr);
                        continue;
                    }
                    ...
                    uploadFailures++;
                    console.warn('Review image upload failed:', e);
                }
            }

            // If user selected images but none uploaded, surface an error and abort insert
            if (reviewFiles.length > 0 && urls.length === 0) {
                setReviewUploadError('Failed to upload images. Please try again.');
                return;
            }
            ...
            const { error } = await supabase.from('user_reviews').insert([payloadBase]);
            if (error) throw error;

            // Update summary counters locally
        ...
        <div className="font-dm-sans w-full bg-cover bg-white phone:pt-[210px] tablet:pt-[220px] laptop:pt-[161px] phone:pb-40 tablet:pb-32 laptop:pb-24 z-0">
            ...
            <div className="max-w-[1200px] mx-auto mt-8 w-full laptop:px-2 phone:p-2 tablet:p-2">
                <div className={`border border-black ${reviews && reviews.length > 0 ? 'border-b-0 rounded-b-none' : ''} rounded-md overflow-hidden`}>
                    ...
                    {/* Divider under header (outside the flex row so it spans full width) */}
                    ...
                    {isReviewFormOpen && (
                        <div className="px-4 pb-4 tablet:px-6 tablet:pb-6 pt-4">
                            <div className="space-y-3">
                                ...
                                <div className="flex flex-col gap-2">
                                    <div className="flex items-center gap-4">
                                        {/* accept only PNG, JPG/JPEG, SVG to match UploadDesign */}
                                        <input
                                    ...
                                    return (
                                        <div key={rev.id} className="py-5">
                                            ...
                                            <div className="flex items-start gap-3">
                                                ...
                                                {/* Body */}
                                                <div className="flex-1 min-w-0">
                                                    {/* Header: author name, verified badge */}
                                                    ...
                                                    {/* Rating stars */}
                                                    <div className="mt-2 flex items-center ml-[-50px] gap-1">
                                                        {Array.from({ length: 5 }).map((_, i) => (
                                                    ...
                                                    {/* Review text */}
                                                    {rev.comment && (
                                                        <p className="mt-3 text-[14px] ml-[-50px] font-dm-sans text-[#111233] break-words">{rev.comment}</p>
                                                    )}
                                                    {/* Review images */}
                                                    {images.length > 0 && (
