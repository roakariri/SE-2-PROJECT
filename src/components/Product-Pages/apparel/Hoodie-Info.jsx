...
const Hoodie = () => {
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Hoodie-Info] recently_viewed update error:', updError);
                }

                if (Array.isArray(updData) && updData.length > 0) {
                    if (process.env.NODE_ENV !== 'production') {
                        console.debug('[Hoodie-Info] recently_viewed updated', { productId, userId, row: updData?.[0]?.id });
                    }
                ...
                if (insError) {
                    console.warn('[Hoodie-Info] recently_viewed insert error:', insError);
                } else {
                    if (process.env.NODE_ENV !== 'production') {
                        console.debug('[Hoodie-Info] recently_viewed inserted', { productId, userId, row: insData?.[0]?.id });
                    }
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Hoodie-Info] recently_viewed log error:', err);
            }
        };
    ...
    useEffect(() => {
        ...
        const resolveImage = async () => {
            ...
            try {
                ...
                if (isMounted) setImageSrc('/apparel-images/hoodie.png');
            } catch (err) {
    ...
    // Build thumbnails: hoodie primary + color variants + fallbacks
    useEffect(() => {
        let isMounted = true;
        ...
        const buildThumbnails = async () => {
            const results = [];
            results.push('/apparel-images/hoodie.png');
            const desired = ['hoodie-black', 'hoodie-gray', 'hoodie-white'];
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Hoodie-Info] reviews query error:', error.message || error);
                ...
                console.error('[Hoodie-Info] unexpected error fetching reviews:', err);
            ...
            if (!cartMatched) {
                ...
                try {
                    if (uploadedFileMetas && uploadedFileMetas.length > 0) {
                        ...
                        if (ids.length > 0) {
                            try {
                                ...
                                if (tryFileId.error) console.warn('Hoodie: attach by file_id failed, will try id column:', tryFileId.error);
                                else if ((tryFileId.data?.length ?? 0) === 0) console.warn('Hoodie: attach by file_id affected no rows, will try id column');
                            } catch (e) { console.warn('Hoodie: unexpected error attaching by file_id', e); }

                            try {
                                const tryId = await supabase.from('uploaded_files').update({ cart_id: cartId }).in('id', ids);
                                if (tryId.error) console.warn('Hoodie: attach by id failed:', tryId.error);
                            } catch (e) { console.warn('Hoodie: unexpected error attaching by id', e); }
                        }
                    }
                    ...
                    try {
                        ...
                        if (fallbackErr) console.warn('Hoodie: fallback attach failed', fallbackErr);
                    } catch (fb) { console.warn('Hoodie: fallback attach exception', fb); }
                } catch (e) {
                    console.warn('Failed to attach uploaded_files to cart row (Hoodie):', e);
                }
            }
                    ...
                    <div className="bg-white w-full tablet:w-[573px] h-auto">
                        <div className="rounded-md p-6 h-full flex flex-col">
                            <div className="relative w-full h-64 tablet:h-[480px] flex-1 flex items-center justify-center bg-[#f7f7f7]">
                                <img src={imageSrc || "/apparel-images/hoodie.png"} alt="" className="w-full max-h-64 tablet:max-h-[420px] object-contain" />
                                <button
                                    type="button"
