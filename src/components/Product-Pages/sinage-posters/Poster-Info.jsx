...
const Poster = () => {
 // Optional: Scroll to top on mount
    ...
    // This helps when joined cart_variants rows are missing nested product_variant_values.
    useEffect(() => {
        const tryHydrateFromNav = () => {
            try {
                ...
                console.debug('[Poster] tryHydrateFromNav failed', e);
            }
            return false;
        };

        // Run once when variantGroups become available or when entering edit-from-cart mode
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Poster-Info] reviews query error:', error.message || error);
                ...
                console.error('[Poster-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
    ...
    const submitReview = async () => {
        ...
        try {
            ...
            console.error('[Poster-Info] submit review error:', err);
