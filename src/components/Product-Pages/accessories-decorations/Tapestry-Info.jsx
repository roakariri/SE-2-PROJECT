 ...
 // Optional: Scroll to top on mount
    ...
    const tryGetPublic = async (bucket, path) => {
        try {
            ...
            console.debug(`[Tapestry] Failed to get public URL for ${bucket}/${path}:`, err);
        }
        return null;
    };
    ...
    const buildThumbnails = async () => {
        const thumbs = [];
        // Main image
        const mainUrl = await tryGetPublic('accessoriesdecorations-images', 'tapestry.png');
        thumbs.push(mainUrl || '/accessories-images/tapestry.png');

        // Additional thumbnails (if any)
        for (let i = 1; i < 4; i++) {
            const url = await tryGetPublic('accessoriesdecorations-images', `tapestry-${i}.png`);
            thumbs.push(url);
        }
        setThumbnails(thumbs);
        // Set initial active thumb to first valid one
        const firstValid = thumbs.findIndex(t => t);
        if (firstValid >= 0) {
            setActiveThumb(firstValid);
            if (!imageSrc && thumbs[firstValid]) setImageSrc(thumbs[firstValid]);
        }
    };
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Tapestry-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Tapestry-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Tapestry-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    // Try to hydrate from navigation state first (fast-path), else restore variants & dimensions by cart_id
    useEffect(() => {
        const tryHydrateFromNav = () => {
            ...
            } catch (e) { console.debug('[tryHydrateFromNav][Tapestry] failed', e); }
            return false;
        };
        ...
        const loadCart = async () => {
            ...
            } catch (e) { console.debug('[EditCart] restore tapestry cart failed', e); }
        };
        loadCart();
    ...
    useEffect(() => {
        ...
        const buildThumbnails = async () => {
            ...
            results.push('/accessories-images/tapestry.png');

            // desired variant thumbnails
            const desired = ['tapestry-1', 'tapestry-2', 'tapestry-3'];
    ...
    useEffect(() => {
        const fetchStockInfo = async () => {
            console.debug('[Stock][Tapestry] fetchStockInfo start', { productId, variantGroupsLen: variantGroups.length, selectedVariants });
            if (!productId || !variantGroups.length) {
                console.debug('[Stock][Tapestry] no productId or no variantGroups');
                setStockInfo(null);
                return;
            }
            const variantIds = Object.values(selectedVariants).map(v => v?.id).filter(Boolean);
            console.debug('[Stock][Tapestry] variantIds computed', { variantIds });
            if (variantIds.length !== variantGroups.length) {
                console.debug('[Stock][Tapestry] not all variants selected', { variantIdsLen: variantIds.length, groups: variantGroups.length });
                setStockInfo(null);
                return;
            }
            ...
            const { data: combinations, error: combError } = await supabase
                ...
                .eq('product_id', productId);
            console.debug('[Stock][Tapestry] fetched combinations', { combinationsLength: (combinations || []).length, combError });
            if (combError) { console.debug('[Stock][Tapestry] combination query error', combError); setStockInfo(null); return; }
            ...
            console.debug('[Stock][Tapestry] match result', { match });
            if (!match) { console.debug('[Stock][Tapestry] no match found'); setStockInfo(null); return; }
            ...
            const { data: inventory, error: invError } = await supabase
                ...
                .single();
            console.debug('[Stock][Tapestry] inventory query result', { inventory, invError });
            if (invError || !inventory) { console.debug('[Stock][Tapestry] no inventory or error', invError); setStockInfo(null); return; }
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Tapestry-Info] reviews query error:', error.message || error);
                ...
                console.error('[Tapestry-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
                ...
                if (uploadedFileMetas && uploadedFileMetas.length > 0) {
                    ...
                    if (ids.length > 0) {
                        try {
                            ...
                            if (res1.error) {
                                console.warn('[Tapestry] attach by file_id failed, trying id column:', res1.error);
                                // Fallback to id column
                                const res2 = await supabase.from('uploaded_files').update({ cart_id: editingCartId }).in('id', ids);
                                if (res2.error) console.warn('[Tapestry] attach by id fallback failed:', res2.error);
                                else if ((res2.data?.length ?? 0) === 0) console.warn('[Tapestry] attach: no rows linked for ids (file_id/id):', ids);
                            } else if ((res1.data?.length ?? 0) === 0) {
                                console.warn('[Tapestry] attach: no rows linked for file_id:', ids);
                            } else {
                                console.debug('[Tapestry] attached uploaded files by file_id:', res1.data?.length ?? 0);
                            }
                        } catch (err) {
                            console.warn('[Tapestry] Failed to link uploaded_files by ids (both columns):', err);
                        }
                    }
                }
                                    ...
                                    {printingGroup.values.map(val => {
                                        ...
                                        return (
                                            <button
                                                type="button"
                                                key={val.id}
                                                className={`px-4 py-2 rounded ${isSelected ? 'bg-gray-200 text-gray-500 font-bold border border-gray-500' : 'bg-white text-[#111233] border border-[#111233]'}`}
                                                onClick={() => selectVariant(printingGroup.id, val)}
                                            >
                ...
                <div className="border border-black rounded-md overflow-hidden">
                    ...
                    >
                        <ul className="text-sm text-gray-700 space-y-2 text-black text-[16px] font-dm-sans">
                            ...
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Material: Satin</p></li>
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Size: 75x100 cm, 120x150 cm, 150x130 cm</p></li>
