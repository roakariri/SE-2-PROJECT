...
const ButtonPin = () => {
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Button-Pin-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Button-Pin-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Button-Pin-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    useEffect(() => {
        const tryHydrateFromNav = () => {
            try {
                ...
                return true;
            } catch (e) { console.debug('[tryHydrateFromNav][ButtonPin] failed', e); return false; }
        };
        ...
        const loadCart = async () => {
            ...
            } catch (e) { console.debug('[EditCart] restore pin cart failed', e); }
        };
        loadCart();
    ...
    useEffect(() => {
        const fetchStockInfo = async () => {
            console.debug('[Stock][ButtonPin] fetchStockInfo start', { productId, variantGroupsLen: variantGroups.length, selectedVariants });
            if (!productId || !variantGroups.length) {
                console.debug('[Stock][ButtonPin] no productId or no variantGroups');
                setStockInfo(null);
                return;
            }
            const variantIds = Object.values(selectedVariants).map(v => v?.id).filter(Boolean);
            console.debug('[Stock][ButtonPin] variantIds computed', { variantIds });
            if (variantIds.length !== variantGroups.length) {
                console.debug('[Stock][ButtonPin] not all variants selected', { variantIdsLen: variantIds.length, groups: variantGroups.length });
                setStockInfo(null);
                return;
            }
            ...
            const { data: combinations, error: combError } = await supabase
                ...
                .eq('product_id', productId);
            console.debug('[Stock][ButtonPin] fetched combinations', { combinationsLength: (combinations || []).length, combError });
            if (combError) { console.debug('[Stock][ButtonPin] combination query error', combError); setStockInfo(null); return; }
            ...
            console.debug('[Stock][ButtonPin] match result', { match });
            if (!match) { console.debug('[Stock][ButtonPin] no match found'); setStockInfo(null); return; }
            ...
            const { data: inventory, error: invError } = await supabase
                ...
                .single();
            console.debug('[Stock][ButtonPin] inventory query result', { inventory, invError });
            if (invError || !inventory) { console.debug('[Stock][ButtonPin] no inventory or error', invError); setStockInfo(null); return; }
    ...
    useEffect(() => {
        ...
        const buildThumbnails = async () => {
            ...
            results.push('/accessories-images/button-pins.png');

            // desired variant thumbnails
            const desired = ['button-pins-1', 'button-pins-2', 'button-pins-3'];
            ...
            const fallbacks = ['/accessories-images/button-pins.png', '/accessories-images/button-pins-1.png', '/accessories-images/button-pins-2.png', '/logo-icon/logo.png'];
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Button-Pin-Info] reviews query error:', error.message || error);
                ...
                console.error('[Button-Pin-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
                ...
                if (uploadedFileMetas && uploadedFileMetas.length > 0) {
                    ...
                    if (ids.length > 0) {
                        try {
                            ...
                            if (res1.error) {
                                console.warn('[ButtonPin] attach by file_id failed, trying id column:', res1.error);
                                // Fallback to id column
                                const res2 = await supabase.from('uploaded_files').update({ cart_id: editingCartId }).in('id', ids);
                                if (res2.error) console.warn('[ButtonPin] attach by id fallback failed:', res2.error);
                                else if ((res2.data?.length ?? 0) === 0) console.warn('[ButtonPin] attach: no rows linked for ids (file_id/id):', ids);
                            } else if ((res1.data?.length ?? 0) === 0) {
                                console.warn('[ButtonPin] attach: no rows linked for file_id:', ids);
                            } else {
                                console.debug('[ButtonPin] attached uploaded files by file_id:', res1.data?.length ?? 0);
                            }
                        } catch (err) {
                            console.warn('[ButtonPin] Failed to link uploaded_files by ids (both columns):', err);
                        }
                    }
                }
