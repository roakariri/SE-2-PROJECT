...
const Blanket = () => {
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Blanket-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Blanket-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Blanket-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    useEffect(() => {
        const fetchStockInfo = async () => {
            console.debug('[Stock][Blanket] fetchStockInfo start', { productId, variantGroupsLen: variantGroups.length, selectedVariants });
            if (!productId || !variantGroups.length) {
                console.debug('[Stock][Blanket] no productId or no variantGroups');
                setStockInfo(null);
                return;
            }
            const variantIds = Object.values(selectedVariants).map(v => v?.id).filter(Boolean);
            console.debug('[Stock][Blanket] variantIds computed', { variantIds });
            if (variantIds.length !== variantGroups.length) {
                console.debug('[Stock][Blanket] not all variants selected', { variantIdsLen: variantIds.length, groups: variantGroups.length });
                setStockInfo(null);
                return;
            }
            ...
            const { data: combinations, error: combError } = await supabase
                ...
                .eq('product_id', productId);
            console.debug('[Stock][Blanket] fetched combinations', { combinationsLength: (combinations || []).length, combError });
            if (combError) { console.debug('[Stock][Blanket] combination query error', combError); setStockInfo(null); return; }
            ...
            console.debug('[Stock][Blanket] match result', { match });
            if (!match) { console.debug('[Stock][Blanket] no match found'); setStockInfo(null); return; }
            ...
            const { data: inventory, error: invError } = await supabase
                ...
                .single();
            console.debug('[Stock][Blanket] inventory query result', { inventory, invError });
            if (invError || !inventory) { console.debug('[Stock][Blanket] no inventory or error', invError); setStockInfo(null); return; }
    ...
    // Fast-path: hydrate selectedVariants & quantity from navigation state (cartRow)
    // This runs once variantGroups are available and attempts tolerant name/id matching
    // to avoid waiting on the DB restore when possible.
    useEffect(() => {
        try {
            ...
            for (const nv of navVariants) {
                ...
                if (match) hydr[String(group.id)] = match;
            }

            if (Object.keys(hydr).length) {
                setSelectedVariants(hydr);
                if (Number(cartRow.quantity) > 0) setQuantity(Number(cartRow.quantity));
                setFromCart(true);
                setEditingCartId(cartRow.cart_id || editingCartId);
            }
        } catch (e) {
            console.debug('[tryHydrateFromNav][Blanket] failed', e);
        }
    ...
    useEffect(() => {
        ...
        const buildThumbnails = async () => {
            ...
            results.push('/accessories-images/blanket.png');

            // desired variant thumbnails
            const desired = ['blanket-1', 'blanket-2', 'blanket-3'];
            ...
            const fallbacks = ['/accessories-images/blanket.png', '/accessories-images/blanket-1.png', '/accessories-images/blanket-2.png', '/logo-icon/logo.png'];
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Blanket-Info] reviews query error:', error.message || error);
                ...
                console.error('[Blanket-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
                ...
                <div className="border border-black rounded-md overflow-hidden">
                    ...
                    >
                        <ul className="text-sm text-gray-700 space-y-2 text-black text-[16px] font-dm-sans">
                            ...
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Material: Polyester Fleece</p></li>
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Size: 50x35 cm, 90x73 cm, 150x130 cm</p></li>
