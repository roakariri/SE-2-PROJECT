   ...
   // Optional: Scroll to top on mount
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Shaker-Keychain-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Shaker-Keychain-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Shaker-Keychain-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    useEffect(() => {
        const fetchEditCart = async () => {
            ...
            try {
                ...
                if (Array.isArray(data.cart_variants) && data.cart_variants.length) {
                    const vMap = {};
    ...
    useEffect(() => {
        ...
        const buildThumbnails = async () => {
            ...
            results.push('/accessories-images/shaker-keychain.png');

            // desired variant thumbnails
            const desired = ['shaker-keychain-1', 'shaker-keychain-2', 'shaker-keychain-3'];
            ...
            const fallbacks = ['/accessories-images/shaker-keychain.png', '/accessories-images/shaker-keychain-1.png', '/accessories-images/shaker-keychain-2.png', '/logo-icon/logo.png'];
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Shaker-Keychain-Info] reviews query error:', error.message || error);
                ...
                console.error('[Shaker-Keychain-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
        ...
        const fetchSizeDimensions = async () => {
            ...
            try {
                ...
                if (error) {
                    ...
                    setLength(data.min_length || 0);
                    setWidth(data.min_width || 0);
                }
            } catch (err) {
                if (!isMounted) return;
                ...
                <div className="flex flex-col tablet:flex-row laptop:gap-2 tablet:gap-[50px] phone:p-2 tablet:p-2 justify-center w-full items-stretch">
                    ...
                    {/* Right: Details */}
                     <div className="border border-black rounded-md p-6 w-full tablet:w-[601px] h-[732px] flex flex-col overflow-y-auto pr-2">
                        ...
                        {/* ACRYLIC PIECES / QUANTITY (from variant group) */}
                        <div className="mb-6">
                            <div className="text-[16px] font-semibold text-gray-700 mb-2">ACRYLIC PIECES QUANTITY (CUSTOMIZABLE)</div>
                            {piecesRow && (
                                <div className="flex items-center gap-3">
                                    {piecesRow.values.map(val => {
                                        const isSelected = selectedVariants[piecesRow.id]?.id === val.id;
                                        return (
                                            <button
                                                key={val.id}
                                                type="button"
                                                onClick={() => {
                                                    // If the pieces/quantity label contains a number (e.g., "5 pcs"), set quantity accordingly
                                                    const numMatch = String(val.name || '').match(/(\d+)/);
                                                    if (numMatch) setQuantity(Number(numMatch[1]));
                                                    selectVariant(piecesRow.id, val);
                                                }}
                                                className={`px-3 py-1 rounded hover:bg-gray-50 focus:outline-none focus:ring-0 ${isSelected ? 'bg-gray-200 text-gray-700 font-semibold border border-[#111233]' : 'bg-white text-[#111233] border border-[#111233]'}`}
                                            >
                                                {val.name}
                                            </button>
                                        );
