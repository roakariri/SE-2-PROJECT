...
const PhoneHolder = () => {
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Phone-Holder-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Phone-Holder-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Phone-Holder-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    useEffect(() => {
        const tryHydrateFromNav = () => {
            try {
                ...
                setEditingCartId(cartRow.cart_id);
                return true;
            } catch (e) { console.debug('[tryHydrateFromNav][PhoneHolder] failed', e); return false; }
        };
    ...
    useEffect(() => {
        const fetchStockInfo = async () => {
            console.debug('[Stock][PhoneHolder] fetchStockInfo start', { productId, variantGroupsLen: variantGroups.length, selectedVariants });
            if (!productId || !variantGroups.length) {
                console.debug('[Stock][PhoneHolder] no productId or no variantGroups');
                setStockInfo(null);
                return;
            }
            const variantIds = Object.values(selectedVariants).map(v => v?.id).filter(Boolean);
            console.debug('[Stock][PhoneHolder] variantIds computed', { variantIds });
            if (variantIds.length !== variantGroups.length) {
                console.debug('[Stock][PhoneHolder] not all variants selected', { variantIdsLen: variantIds.length, groups: variantGroups.length });
                setStockInfo(null);
                return;
            }
            ...
            const { data: combinations, error: combError } = await supabase
                ...
                .eq('product_id', productId);
            console.debug('[Stock][PhoneHolder] fetched combinations', { combinationsLength: (combinations || []).length, combError });
            if (combError) { console.debug('[Stock][PhoneHolder] combination query error', combError); setStockInfo(null); return; }
            ...
            console.debug('[Stock][PhoneHolder] match result', { match });
            if (!match) { console.debug('[Stock][PhoneHolder] no match found'); setStockInfo(null); return; }
            ...
            const { data: inventory, error: invError } = await supabase
                ...
                .single();
            console.debug('[Stock][PhoneHolder] inventory query result', { inventory, invError });
            if (invError || !inventory) { console.debug('[Stock][PhoneHolder] no inventory or error', invError); setStockInfo(null); return; }
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Phone-Holder-Info] reviews query error:', error.message || error);
                ...
                console.error('[Phone-Holder-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
