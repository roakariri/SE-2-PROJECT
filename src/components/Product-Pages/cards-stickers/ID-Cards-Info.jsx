...
const IDCards = () => {
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[ID-Cards-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[ID-Cards-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[ID-Cards-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    // Build thumbnails: 1) main product image as first thumb, 2) id-cards variants from cards-stickers-images storage, 3) fallbacks
    useEffect(() => {
        let isMounted = true;
        ...
        const buildThumbnails = async () => {
            ...
            // first thumbnail: use the resolved imageSrc from Supabase
            if (imageSrc && imageSrc !== '/logo-icon/logo.png') {
                results.push(imageSrc);
            } else {
                // fallback to local image if Supabase image not available
                results.push('/images/id-cards.png');
            }


            // desired variant thumbnails
            const desired = ['id-cards-white', 'id-cards-black', 'id-cards-transparent'];
            ...
            const fallbacks = ['/cards/id-cards.png', '/cards/id-cards-1.png', '/cards/id-cards-2.png', '/logo-icon/logo.png'];
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[ID-Cards-Info] reviews query error:', error.message || error);
                ...
                console.error('[ID-Cards-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
    ...
    const submitReview = async () => {
        ...
        try {
            ...
            console.error('[ID-Cards-Info] submit review error:', err);
    ...
    const handleAddToCart = async () => {
        ...
        const userId = session?.user?.id ?? await (async () => { const { data: { user } } = await supabase.auth.getUser(); return user?.id; })();
        if (!userId) {
                ...
                <div className="border border-black rounded-md overflow-hidden">
                    ...
                    >
                        <ul className="text-sm text-gray-700 space-y-2 text-black text-[16px] font-dm-sans">
                            ...
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Material: PVC Card</p></li>
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">GSM: 300 gsm</p></li>
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Size: 6 x 8.5 cm</p></li>
