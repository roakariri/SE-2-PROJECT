...
const Postcards = () => {
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Postcards-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Postcards-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Postcards-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    // Build thumbnails: 1) main product image as first thumb, 2) postcards variants from cards-stickers-images storage, 3) fallbacks
    useEffect(() => {
        let isMounted = true;
        ...
        const buildThumbnails = async () => {
            ...
            // first thumbnail: ensure postcards.png is always first
            results.push('/cards/postcards.png');

            // desired variant thumbnails
            const desired = ['postcards-white', 'postcards-black', 'postcards-transparent'];
            ...
            const fallbacks = ['/cards/postcards.png', '/cards/postcards-1.png', '/cards/postcards-2.png', '/logo-icon/logo.png'];
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Postcards-Info] reviews query error:', error.message || error);
                ...
                console.error('[Postcards-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
    ...
    const submitReview = async () => {
        ...
        try {
            ...
            console.error('[Postcards-Info] submit review error:', err);
                ...
                <div className="border border-black rounded-md overflow-hidden">
                    ...
                    >
                        <ul className="text-sm text-gray-700 space-y-2 text-black text-[16px] font-dm-sans">
                            ...
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Material: Premium Thick Card (300 gsm)</p></li>
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Size: 4 x 6 inches</p></li>
