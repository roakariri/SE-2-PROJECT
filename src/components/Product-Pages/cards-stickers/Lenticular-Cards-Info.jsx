...
const LenticularCards = () => {
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Lenticular-Cards-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Lenticular-Cards-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Lenticular-Cards-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Lenticular-Cards-Info] reviews query error:', error.message || error);
                ...
                console.error('[Lenticular-Cards-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviewsCount(0);
                setAverageRating(null);
                setReviews([]);
    ...
    const submitReview = async () => {
        ...
        try {
            ...
            console.error('[Lenticular-Cards-Info] submit review error:', err);
                ...
                <div className="border border-black rounded-md overflow-hidden">
                    ...
                    >
                        <ul className="text-sm text-gray-700 space-y-2 text-black text-[16px] font-dm-sans">
                            ...
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Material: Lenticular Sheet</p></li>
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Size: 5.5x8.5 cm, 6.3x8.8 cm</p></li>
