...
const ThankyouCards = () => {
    ...
    useEffect(() => {
        const logRecentlyViewed = async () => {
            try {
                ...
                if (updError) {
                    console.warn('[Thankyou-Cards-Info] recently_viewed update error:', updError);
                }
                ...
                if (insError) {
                    console.warn('[Thankyou-Cards-Info] recently_viewed insert error:', insError);
                } else {
                    hasLoggedViewRef.current = true;
                }
            } catch (err) {
                console.warn('[Thankyou-Cards-Info] recently_viewed log error:', err);
            }
        };
        logRecentlyViewed();
    ...
    // Build thumbnails: 1) main product image as first thumb, 2) thankyou-cards variants from cards-stickers-images storage, 3) fallbacks
    useEffect(() => {
        let isMounted = true;
        ...
        const buildThumbnails = async () => {
            ...
            // first thumbnail: ensure thankyou-cards.png is always first
            results.push('/cards/thankyou-cards.png');

            // desired variant thumbnails
            const desired = ['thankyou-cards-white', 'thankyou-cards-black', 'thankyou-cards-transparent'];
            ...
            const fallbacks = ['/cards/thankyou-cards.png', '/cards/thankyou-cards-1.png', '/cards/thankyou-cards-2.png', '/logo-icon/logo.png'];
    ...
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            if (!productId) return;
            console.log('[Thankyou-Cards-Info] fetching reviews for product:', productId);
            ...
            try {
                ...
                if (!isMounted) return;
                if (error) {
                    console.warn('[Thankyou-Cards-Info] reviews query error:', error.message || error);
                    ...
                    console.log('[Thankyou-Cards-Info] fetched reviews:', data.length);
                    setReviews(data);
                    ...
                    console.log('[Thankyou-Cards-Info] no reviews found');
                    setReviewsAvailable(true);
                    setReviews([]);
                    setReviewsCount(0);
                    setAverageRating(null);
                ...
                console.error('[Thankyou-Cards-Info] unexpected error fetching reviews:', err);
                setReviewsAvailable(false);
                setReviews([]);
                setReviewsCount(0);
                setAverageRating(null);
    ...
    const submitReview = async () => {
        ...
        try {
            ...
            console.log('[Thankyou-Cards-Info] review submitted successfully');
            // Refresh reviews
            ...
            console.error('[Thankyou-Cards-Info] error submitting review:', err);
    ...
    useEffect(() => {
        const tryHydrateFromNav = () => {
            try {
                ...
                console.debug('[ThankyouCards] tryHydrateFromNav failed', e);
            }
            return false;
        };
    ...
    // Add to Cart logic from Tote Bag
        ...
        // Check if design files are uploaded
        ...
        try {
            ...
            if (fromCart && editingCartId) {
                ...
                const newTotal = (Number(unitPrice) || Number(price) || 0) * newQuantity;
            ...
            // Fallback attachment for uploaded files
            if (uploadedFileMetas && uploadedFileMetas.length > 0) {
                try {
                    const attachmentPromises = uploadedFileMetas.map(async (fileMeta) => {
                        const { error: attachError } = await supabase
                            .from("uploaded_files")
                            .update({ cart_id: cartId })
                            .eq("file_id", fileMeta.file_id)
                            .eq("user_id", userId);

                        if (attachError) {
                            console.error("Failed to attach file to cart:", attachError);
                        }
                    });

                    await Promise.all(attachmentPromises);
                } catch (attachErr) {
                    console.error("Error in fallback attachment:", attachErr);
                }
            }
        ...
        <div className="font-dm-sans w-full bg-cover bg-white phone:pt-[210px] tablet:pt-[220px] laptop:pt-[161px] phone:pb-40 tablet:pb-32 laptop:pb-24 z-0">
                    ...
                    <div className="border border-black rounded-md p-6 w-full tablet:w-[601px] h-[732px] flex flex-col overflow-y-auto pr-2">
                        ...
                        <div className="mb-6">
                            ...
                            {cutGroup ? (
                                ...
                                <div className="text-sm text-gray-500">No cut style options</div>
                            )}
                        </div>

                        {/* QUANTITY from variant group (if available) */}
                        <div className="mb-6">
                            ...
                            {quantityGroup ? (
                                ...
                                <div className="text-sm text-gray-500">No quantity options</div>
                            )}
                        </div>
                ...
                <div className="border border-black rounded-md overflow-hidden">
                    ...
                    >
                        <ul className="text-sm text-gray-700 space-y-2 text-black text-[16px] font-dm-sans">
                            ...
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Material: Premium Thick Card (250 gsm)</p></li>
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">GSM: 150 gsm</p></li>
                            <li><p className="m-0 font-normal text-black text-[16px] font-dm-sans">Size: 3.5x2 inches</p></li>
            ...
            <div className="max-w-[1200px] mx-auto mt-8 w-full laptop:px-2 phone:p-2 tablet:p-2">
                <div className={`border border-black ${reviews && reviews.length > 0 ? 'border-b-0 rounded-b-none' : ''} rounded-md overflow-hidden`}>
                    ...
                    {!isReviewFormOpen && reviews && reviews.length > 0 && (
                        <div className="pt-4 px-4 tablet:px-6">
                            <div className="flex items-center font-medium gap-2 text-[#111233] text-sm">
                                <span>Filter by:</span>
                                <button
                                    type="button"
                                    onClick={() => setStarFilterRating(0)}
                                    className={`px-3 py-1 rounded ${starFilterRating === 0 ? 'bg-gray-200' : 'bg-white'} border border-gray-300 focus:outline-none focus:ring-0`}
                                >
                                    All
                                </button>
                                {[5, 4, 3, 2, 1].map((rating) => (
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded flex items-center gap-1 ${starFilterRating === rating ? 'bg-gray-200' : 'bg-white'} border border-gray-300 focus:outline-none focus:ring-0`}
                                    >
                    ...
                    {isReviewFormOpen && (
                        <div className="px-4 pb-4 tablet:px-6 tablet:pb-6 pt-4">
                            ...
                            <div className="w-full mt-5">
                                <hr className="mt-2 border-t border-gray-300" />
                                <div className="pt-4">
                                    <div className="flex items-center gap-2 font-medium text-[#111233] text-sm">
                                        <span>Filter by:</span>
                                        <button
                                            type="button"
                                            onClick={() => setStarFilterRating(0)}
