  ...
  const [products, setProducts] = useState([]);
         ...
         const fetchProducts = async () => {
           ...
           if (error) {
             ...
             // Only show products whose type's category is 'Packaging'
             const packagingProducts = data.filter(product =>
               product.product_types?.product_categories?.name === 'Packaging'
             );
             setProducts(packagingProducts);
             setFilteredProducts(packagingProducts);
           }
         };
       
         const fetchProductTypes = async () => {
           // Only show product types present in Packaging products
           const typesInPackaging = new Set();
           products.forEach(product => {
             if (product.product_types?.id && product.product_types?.name) {
               typesInPackaging.add(JSON.stringify({ id: product.product_types.id, name: product.product_types.name }));
             }
           });
           const filteredTypes = Array.from(typesInPackaging).map(str => JSON.parse(str));
        ...
        window.scrollTo({ top: 0, left: 0, behavior: 'auto' });
         ...
         return (
           <div className="w-full bg-cover bg-white phone:pt-[210px] tablet:pt-[220px] laptop:pt-[165px] landing-page-container z-0">
             {/* Hero Banner */}
             <div className="flex laptop:h-[425px] phone:h-[210px] flex-col items-center justify-center z-5 bg-[url('/images/packaging-banner.png')] bg-cover bg-center">
               <p className="text-white"><a className="hover:underline hover:text-white text-white" href="/Homepage">Home</a> /</p>
               <h1 className="text-white font-bold">Packaging</h1>
             </div>
                       ...
                       >
                         <div className="relative w-[230px] h-48 mb-4 mx-auto overflow-hidden">
                           <img
                             src={
                               product.image_url
                                 ? supabase.storage.from('packaging-images').getPublicUrl(product.image_url).data.publicUrl
