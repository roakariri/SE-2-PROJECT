  ...
  const [products, setProducts] = useState([]);
      ...
      const fetchProducts = async () => {
        ...
        if (error) {
          ...
          // Only show products whose type's category is 'Signage & Posters'
          const signageProducts = data.filter(product =>
            product.product_types?.product_categories?.name === 'Signage & Posters'
          );
          setProducts(signageProducts);
          setFilteredProducts(signageProducts);
        }
      };
    
      const fetchProductTypes = async () => {
        // Only show product types present in Signage & Posters products
        const typesInSignage = new Set();
        products.forEach(product => {
          if (product.product_types?.id && product.product_types?.name) {
            typesInSignage.add(JSON.stringify({ id: product.product_types.id, name: product.product_types.name }));
          }
        });
        const filteredTypes = Array.from(typesInSignage).map(str => JSON.parse(str));
    ...
    useEffect(() => {
      ...
      return (
        <div className="w-full bg-cover bg-white phone:pt-[210px] tablet:pt-[220px] laptop:pt-[165px] landing-page-container z-0">
          {/* Hero Banner */}
          <div className="flex laptop:h-[425px] phone:h-[210px] flex-col items-center justify-center z-5 bg-[url('/images/signage-poster-banner.png')] bg-cover bg-center">
            <p className="text-white"><a className="hover:underline hover:text-white text-white" href="/Homepage">Home</a> /</p>
            <h1 className="text-white font-bold">Signage & Posters</h1>
          </div>
                    ...
                    >
                      <div className="relative w-[230px] h-48 mb-4 mx-auto overflow-hidden">
                        <img
                          src={
                            product.image_url
                              ? supabase.storage.from('signage-posters-images').getPublicUrl(product.image_url).data.publicUrl
...
export default SignagesPostersCatalog;
