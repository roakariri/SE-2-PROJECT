  ...
  const [products, setProducts] = useState([]);
         ...
         const fetchProducts = async () => {
           ...
           if (error) {
             ...
             setProducts(threeDPrintProducts);
             setFilteredProducts(threeDPrintProducts);
           }
         };
         ...
         const fetchProductTypes = async () => {
           ...
           products.forEach(product => {
             if (product.product_types?.id && product.product_types?.name) {
               typesInThreeDPrints.add(JSON.stringify({ id: product.product_types.id, name: product.product_types.name }));
             }
           });
           const filteredTypes = Array.from(typesInThreeDPrints).map(str => JSON.parse(str));
        ...
        window.scrollTo({ top: 0, left: 0, behavior: 'auto' });
         ...
         return (
           <div className="w-full bg-cover bg-white phone:pt-[210px] tablet:pt-[220px] laptop:pt-[165px] landing-page-container z-0">
             {/* Hero Banner */}
             <div className="flex laptop:h-[425px] phone:h-[210px] flex-col items-center justify-center z-5 bg-[url('/images/3dprints-banner.png')] bg-cover bg-center">
               <p className="text-white"><a className="hover:underline hover:text-white text-white" href="/Homepage">Home</a> /</p>
               <h1 className="text-white font-bold">3D Print Services</h1>
             </div>
                       ...
                       >
                         <div className="relative w-[230px] h-48 mb-4 mx-auto overflow-hidden">
                           <img
                             src={
                               product.image_url
                                 ? supabase.storage.from('3d-prints-images').getPublicUrl(product.image_url).data.publicUrl
...
export default ThreeDPrintsCatalog;
