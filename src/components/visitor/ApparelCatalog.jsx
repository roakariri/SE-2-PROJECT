...
const ApparelCatalog = () => {
  ...
  const fetchProductTypes = async () => {
    // Only show product types present in Apparel products
    const typesInApparel = new Set();
    products.forEach(product => {
      if (product.product_types?.id && product.product_types?.name) {
        typesInApparel.add(JSON.stringify({ id: product.product_types.id, name: product.product_types.name }));
      }
    });
    const filteredTypes = Array.from(typesInApparel).map(str => JSON.parse(str));
