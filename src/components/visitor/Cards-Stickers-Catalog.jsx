 ...
 const [products, setProducts] = useState(() => __initialCache?.products || []);
  ...
  const [favoriteIds, setFavoriteIds] = useState(() => __initialCache?.favoriteIds || []);
       ...
       const fetchProductTypes = async () => {
         // Only show product types present in Cards & Stickers products
         const typesInCardsStickers = new Set();
         products.forEach(product => {
           if (product.product_types?.id && product.product_types?.name) {
             typesInCardsStickers.add(JSON.stringify({ id: product.product_types.id, name: product.product_types.name }));
           }
         });
         const filteredTypes = Array.from(typesInCardsStickers).map(str => JSON.parse(str));
      ...
      const applySort = (sortValue) => {
        // When 'relevance' is selected we want to restore the default ordering
        // by rebuilding filteredProducts from the canonical `products` list
        // using the current filter state (productType + price).
      ...
      useEffect(() => {
       ...
       return (
         <div className="w-full bg-cover bg-white phone:pt-[210px] tablet:pt-[220px] laptop:pt-[165px] landing-page-container z-0">
           {/* Hero Banner */}
           <div className="flex laptop:h-[425px] phone:h-[210px] flex-col items-center justify-center z-5 bg-[url('/images/cards-stickers-image.png')] bg-cover bg-center">
             <p className="text-white"><a className="hover:underline hover:text-white text-white" href="/Homepage">Home</a> /</p>
             <h1 className="text-white font-bold">Cards & Stickers</h1>
           </div>
...
export default CardsStickersCatalog;
